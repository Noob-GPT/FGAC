...
export default function Input({setChatData, onLoadingChange}: InputProps) {
    ...
    const fixedPrompts: Record<number, string> = {
        ...
        4: '위에서 알려준 바닥면적을 이용해서 화재 및 피난시뮬레이션의 시나리오 작성 기준(제4조 관련)법을 참고해서 수용인원을 산정해주는데 소방법과 건축법으로 각각 계산한 수용인원을 산정해줘.',
        // 가연물 특성조사
        5: '해당 건물 용도에서 일반적인 대표가연물과, 가연물의 특성을 성능기준안전설계 보고서 작성에 용이하도록 알려주는데 이 공간에서의 대표가연물을 선정한 이유에 대한 출처도 꼭 알려줘',
        // 피난인의 특성
        6: '그럼 같은 공간의 조건에서 피난인의 특성은 어떤게 있는지 논문 등을 참고해서 출처와 함께 자세히 알려줘.',
        // 피난 시나리오 작성
        7: '화재 및 피난시뮬레이션의 시나리오 작성 기준 별표1의 시나리오 유형 7가지를 참고해 같은 앞 단계들에서 수집한 유사한 용도의 화재사례를 분석해 시나리오를 자세히 작성해줘.',
        // 7: '이 건물의 용도는 뭐였어? 해당 용도와 유사한 용도의 화재사례를 분석해서 시나리오를 자세히 작성해줘. 이 때, 화재 및 피난시뮬레이션의 시나리오 작성 기준 별표1의 시나리오 유형 7가지를 참고해 같은 앞 단계들에서 수집한 유사한 용도의 화재사례를 분석해 시나리오를 자세히 작성해줘.',
        // 선택 질문
        // 이 7가지 시나리오 중에서 대피 경로 최적화의 시나리오는 몇번 시나리오인지 자세한 이유와 함께 알려줘

        // ASET 계산
        8: 'ASET은 거실 허용 피난 시간으로 a에 바닥면적(m2)의 루트를 곱한 값으로 계산하는데 a는 천장의 높이에 따라 결정하고 천장의 높이가 6m 미만일 때는 2, 6m 이상일 때는 3을 사용해서 계산하고 풀이도 함께 알려줘',
        // RSET 계산
        9: '이번에는 RSET 거실피난시간 계산하는 공식을 차례대로 알려줄게. 각각의 값을 위에서 찾아준 값으로 넣어주고 없는 값의 경우는 용도에 맞게 값을 찾아 넣어 번호에 맞게 차례대로 계산한 값을 자세한 풀이와 함께 알려줘\n' +
            '1. 바닥면적: 바닥면적(m2) 측정(도면에서 찾은 바닥면적으로)\n' +
            '2. 피난대상인원수: 거실면적에 인구밀도를 곱하여 계산하는데 인구밀도는 국내법을 참고해서 용도에 맞는 값을 대입\n' +
            '3. 문 너비: 문들의 너비(m) 값\n' +
            '4. 출구통과시간(t11): 피난대상인원수를 문 폭의 합계에 1.5 곱한 값\n' +
            '5. 실내보행시간(t12): Lx+y를 V로 나눈 값, Lx+y는 가로와 세로 면적의 합으로 가장 긴 피난 경로를 나타내고  V는 보행 속도를 의미하는데 보행 속도는 용도에 따라 달라지며, 사무소나 학교는 1.3m/s, 백화점이나 호텔은 1.0m/s, 병원은 0.5m/s로 설정\n' +
            '6. 거실피난시간: MAX(t11,t12)',
        // 피난 시간 비교
        10: '위에서 구한 ASET 계산의 거실허용피난시간을 RSET 결과 계산 시간을 비교한 후 안전한 피난인지 불안전한 피난인지 정확하게 파악해서 알려줘',
        // 개선안 도출
        11: '불안전한 피난인 경우에는 건물의 구조적 특성, 사용하는 인원, 용도, 대피 인원의 특성을 고려해서 안전한 피난이 나올 수 있도록 개선안을 도출해주고 안전한 피난인 경우에는 개선안을 도출하지 말고 내용만 정리해서 알려줘.',
        // 선택 질문
        // 각 개선안에 대해 비용이 얼마정도 드는지 대략적으로 알려줘.
        // 플래시오버 도달시간
        12: '도면에서 문의 면적과 높이를 찾아주고 플래시오버 도달 시간을 계산해줘. 이때 바브라우스카스 방법을 사용해서 플래시오버를 위한 필요 열방출율을 계산하고, 화재성장 모델은 T-제곱 화재 모델을 사용해서 플래시오버 도달시간을 계산해줘. 화재 성장속도는 도면의 건물 용도에 맞는 연료유형에 따라서 성장계수를 추천해서 대입해줘.',
    };
    ...
    return (
        <>
            <Grid container spacing={2}>
                <Grid size={['2', '7', '11'].includes(stepId || "0") ? 9 : 11}>
                    <TextField
